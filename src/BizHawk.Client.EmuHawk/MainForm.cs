...
//				Util.DebugWriteLine($"  Selecting display size {lastComputedSize}");
		...
		private bool Rewind(ref bool runFrame, long currentTimestamp, out bool returnToRecording)
		{
			...
			if (ForceRewind || Rewinder?.Active == true && (InputManager.ClientControls["Rewind"] || PressRewind))
			{
				if (EmulatorPaused)
				{
					if (_frameRewindTimestamp == 0)
		...
		private IAtrapalhanciaGame atrapalhanciaGame = ((IAtrapalhanciaGame)Activator.CreateInstance(currentGame));

		private void BeforeGameLoaded()
		{
			Player = Microsoft.VisualBasic.Interaction.InputBox("Jogador", "rene ou jotas", "rene").ToLower();
			Instance = this;
			

			RamWatch.watchPath = atrapalhanciaGame.Watch;

			RamWatch.MainThreadInvoker = this.BeginInvoke;
		...
		private void OnGameLoaded()
		{
			...
			server = new Thread(() =>
			{
				...
				listener.Prefixes.Add(Server.url);
				listener.Start();
				Console.WriteLine("Esperando atrapalhancias {0}", Server.url);

				Task listenTask = Server.HandleIncomingConnections(listener, Player, currentAtrapalhancias);
				try
				{
		...
		{
			foreach(var atrapalhancia in currentAtrapalhancias)
			{
				var item = new ToolStripMenuItem(atrapalhancia.Key);
				item.Click += (e, i) =>
				{
					Queues.DirectQueue(atrapalhancia.Key, new Task(() =>
					{
