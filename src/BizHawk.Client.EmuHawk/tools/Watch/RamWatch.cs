...
namespace BizHawk.Client.EmuHawk
{
	public partial class RamWatch : ToolFormBase, IToolFormAutoConfig
	{
		...
		public static Action HideWindow = null;

		public RamWatch()
		{
			HideWindow = () => this.WindowState = FormWindowState.Minimized;
			InitializeComponent();
		...
		private void FrameUpdate()
		{
			...
			if (_watches.Any())
			{
				...
				//Console.WriteLine(WatchListView.All);
				WatchListView.RowCount = _watches.Count;
			}
		}
		...
		private void RecentSubMenu_DropDownOpened(object sender, EventArgs e)
		{
			...
			RecentSubMenu.DropDownItems.AddRange(Config.RecentWatches.RecentMenu((IDialogParent)MainForm, LoadFileFromRecent, "Watches"));
		}
		...
		private void RamWatch_Load(object sender, EventArgs e)
		{
			...
			PokeAddressToolBarItem.Enabled =
				FreezeAddressToolBarItem.Enabled =
					MayPokeAllSelected;
			ram = this;
			//ByteWatch.RegenerateHP = () => Poke("health", "8");
		}
		...
		private void WatchListView_Click(object sender, EventArgs e)
		{

		}

		public static RamWatch ram;

		public Action Freeze(string note, int value)
		{
			Cheat cheat = null;
			var watch = Watches.First(w => w.Notes.ToLower() == note.ToLower());
			MainThreadInvoker(() =>
			{
				cheat = new Cheat(watch, value);
				MainForm.CheatList.Add(cheat);
			});
		...
		public void PokeCartRAM(string pointer, string value)
		{
			var cartRAMReference = Watches.First((x) => x.Notes.ToLower() == "cartram");
			var cartRAMPointer = Watch.GenerateWatch(cartRAMReference.Domain, Convert.ToInt64(Hex.HexCartRAMToNumber(pointer)), WatchSize.DWord, WatchDisplayType.Hex, true);
			MainThreadInvoker(() => cartRAMPointer.Poke(value));
		}

		public static Action<Action> MainThreadInvoker;
		public Action FreezeCartRAM(string pointer, string value)
		{
			Cheat cheat = null;
			MainThreadInvoker(() =>
			{
				var cartRAMReference = Watches.First((x) => x.Notes.ToLower() == "cartram");
				var cartRAMPointer = Watch.GenerateWatch(cartRAMReference.Domain, Convert.ToInt64(Hex.HexCartRAMToNumber(pointer)), WatchSize.DWord, WatchDisplayType.Hex, true);
				cheat = new Cheat(cartRAMPointer, Hex.HexCartRAMToNumber(value));
				MainForm.CheatList.Add(cheat);
			});
			return () => MainThreadInvoker(() => MainForm.CheatList.Remove(cheat));
		}

		public Action FreezeCartRAMRange(List<KeyValuePair<string,string>> pointerValueDict)
		{
			var cartRAMReference = Watches.First((x) => x.Notes.ToLower() == "cartram");
			List<Cheat> cheats = new List<Cheat>();
			foreach (var pointerValueKVP in pointerValueDict)
			{
				MainThreadInvoker(() =>
				{
					var cartRAMPointer = Watch.GenerateWatch(cartRAMReference.Domain, Convert.ToInt64(Hex.HexCartRAMToNumber(pointerValueKVP.Key)), WatchSize.DWord, WatchDisplayType.Hex, true);
					cheats.Add(new Cheat(cartRAMPointer, Hex.HexCartRAMToNumber(pointerValueKVP.Value)));
				});
			}

			MainThreadInvoker(() => MainForm.CheatList.AddRange(cheats));
			return () => MainThreadInvoker(() => MainForm.CheatList.RemoveRange(cheats));
		}

		private Random rnd = new Random();
		public void RandomPoke()
		{
			var firstWatch = Watches.ToArray()[rnd.Next(1)];
			MainThreadInvoker(() =>
			{
				var randomCorruptionWatch = Watch.GenerateWatch(firstWatch.Domain, Convert.ToInt64(rnd.Next(16000)), WatchSize.Word, WatchDisplayType.Hex, true);
				var cheat = new Cheat(randomCorruptionWatch, rnd.Next(16000));
				MainForm.CheatList.Add(cheat);
			});
		}
